  ...
  wss.on('connection', async (ws, req) => {
    ...
    if (role === 'display') {
      if (sessionParam && (await sessionExists(sessionParam))) {
        // Rejoin existing session (reconnect)
      ...
      if (sessionParam && (await sessionExists(sessionParam))) {
        ...
        // No session param — implicit session for standalone mode
        sessionId = createSession();
      }
    }
    ...
    if (role === 'controller') {
      ...
      // Notify displays that a controller connected
    ...
    ws.on('message', async (raw) => {
      ...
      if (role === 'display') {
        if (parsed.type === 'state' || parsed.type === 'song' || parsed.type === 'beat') {
          cacheState(sessionId, parsed.type, msg).catch(() => {});
        }
      }

      // Broadcast to local room
      broadcastToRoom(sessionId, msg, role, ws);
      ...
      if (redis) {
        ...
        redis.publish(`ws:relay:${sessionId}`, envelope).catch(() => {});
      }
    });

    ws.on('close', () => {
      room.delete(client);
      console.log(`[ws] ${role} left session ${sessionId} (${room.size} in room)`);
